	...
	public partial class FilamentedExtrasInspector : ShaderGUI
	{
	    ...
	    sealed class SetKeywordDrawer : MaterialPropertyDrawer
	    {
	        ...
	        public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
	        {
	            ...
	            else if (prop.propertyType == ShaderPropertyType.Texture)
	            {
	...
	sealed class SingleLineDrawer : MaterialPropertyDrawer
	{
        ...
        try
        {
            ...
            else
            {
                ...
                if (extraProp != null && extraProp.propertyType == ShaderPropertyType.Color && (extraProp.propertyFlags & ShaderPropertyFlags.HDR) > 0)
                {
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                if (extraProp.propertyType == ShaderPropertyType.Color && (extraProp.propertyFlags & ShaderPropertyFlags.HDR) > 0)
                    editor.TexturePropertyWithHDRColor(label, prop, extraProp, false);
                else
		...
		sealed class RGBSliderDecorator : MaterialPropertyDrawer
		{
			...
			public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
			{
				if (prop.propertyType == ShaderPropertyType.Color || prop.propertyType == ShaderPropertyType.Vector)
				{
					bool isColor = prop.propertyType == ShaderPropertyType.Color;
