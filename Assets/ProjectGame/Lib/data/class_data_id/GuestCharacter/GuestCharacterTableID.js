// GuestCharacterTableID - Pure JavaScript Version
export const GuestCharacterTableID = {
    None: 0,  // デフォルト値（C#互換）
    Kuzuha: 1,  // 葛葉
    Ange: 2,  // アンジュ
    Ryushen: 3,  // 緑仙
...
export const GuestCharacterTableIDExtensions = {
    /**
     * Enumを数値に変換
     */
    toInt(id) {
    ...
    forID(action) {
        ...
        const start = GuestCharacterTableID.Kuzuha;
        const max = GuestCharacterTableID.Max;
        for (let id = start; id < max; id++) {
            if (Object.values(GuestCharacterTableID).includes(id)) {
                action(id);
            }
        }
    },
